...
.token-table{width:100%;border-collapse:collapse;font-variant-numeric:tabular-nums}.token-table th,.token-table td{text-align:left;padding:8px 10px;border-bottom:1px solid #e5e9f0}.token-table th:first-child,.token-table td:first-child{width:90px}.token-text{font-family:ui-monospace,monospace;white-space:pre-wrap}.token-hex{font-family:ui-monospace,monospace;word-break:break-word}.token-empty{color:#697586}
...
<section class="card"><div class="bar"><h2 style="margin:0">Original prompt tokens</h2><span id="promptSummary" class="muted"></span></div><div id="promptTokens"><p class="token-empty">No prompt tokenization recorded.</p></div></section>
...
async function load(){trace=await (await fetch('/trace.json')).json();$('summary').textContent=`${trace.events.length} events · ${trace.checkpoint||'unknown checkpoint'}`;$('slider').max=Math.max(0,trace.events.length-1);renderPromptTokens();drawDots();render(0)}
...
function renderPromptTokens(){
  const tokens=Array.isArray(trace.prompt_tokens)?trace.prompt_tokens:[];
  if(!tokens.length){$('promptTokens').innerHTML='<p class="token-empty">No prompt tokenization recorded.</p>';$('promptSummary').textContent='';return}
  $('promptSummary').textContent=`${tokens.length} tokens · includes BOS`;
  $('promptTokens').innerHTML='<table class="token-table"><thead><tr><th>Token ID</th><th>Token 文本</th><th>Token bytes hex</th></tr></thead><tbody>'+tokens.map(item=>{
    const tokenText=escapeHtml(item.token_text||'');
    const tokenHex=escapeHtml(item.token_bytes_hex||'');
    return `<tr><td><code>${escapeHtml(item.token_id)}</code></td><td class="token-text">${tokenText||'∅'}</td><td class="token-hex"><code>${tokenHex||'∅'}</code></td></tr>`;
  }).join('')+'</tbody></table>';
}
