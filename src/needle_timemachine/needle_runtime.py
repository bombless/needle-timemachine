...
class NeedleRuntime:
    ...
    def hidden_states(self, tokens: Any, **kwargs: Any) -> Any:
        ...
        if hook_context is None:
            hidden_states = self.model.apply(variables, tokens, method=self.model.hidden_states, **kwargs)
        else:
            with hook_context:
                hidden_states = self.model.apply(variables, tokens, method=self.model.hidden_states, **kwargs)
    ...
    def logits(self, tokens: Any, **kwargs: Any) -> Any:
        variables = {"params": self.params}
        self.adapter.tracer.emit("model_input", name="model.input", tensors={"tokens": tokens}, metadata={"shape_only": True})
