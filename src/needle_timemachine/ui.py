...
_HTML = r'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Needle Time Machine</title>
<style>
...
<section class="card"><div class="bar"><h2 style="margin:0">Experiment Playground</h2><span class="badge">Prompt + Tools + System facts</span></div><p class="muted">Choose a starter experiment, edit any input, then run Needle and inspect every generated token.</p><div id="templates" class="template-row"></div>
<div class="editor" style="margin-top:14px"><div class="field"><label for="prompt">Prompt</label><textarea id="prompt"></textarea></div><div class="field"><label for="system">System facts / instructions</label><textarea id="system"></textarea></div><div class="field full"><label for="tools">Tools / JSON Schema</label><textarea id="tools" spellcheck="false"></textarea></div></div>
<div class="bar" style="margin-top:12px"><button class="primary" id="run">▶ Run & Trace</button><label>Top-K <input id="topk" type="number" min="1" max="50" value="5" style="width:65px"></label><label>Max new tokens <input id="maxnew" type="number" min="1" max="256" value="48" style="width:75px"></label><span id="status" class="muted"></span></div><div id="error" class="error"></div></section>
<section id="result" class="hidden"><div class="card"><div class="bar"><h2 style="margin:0">Generated output</h2><span id="resultSummary" class="muted"></span></div><pre id="output"></pre></div>
<div class="card"><div class="bar"><button id="first">|&lt;</button><button id="prev">&lt;</button><button id="next">&gt;</button><button id="last">&gt;|</button><span id="step" class="badge"></span></div><input id="slider" type="range" min="0" max="0" value="0"><div class="timeline"><div class="track" id="track"></div></div></div>
<div class="grid"><section class="card"><h2>Chosen token</h2><div id="chosen" class="chosen"></div><h3>Top-K probabilities</h3><div id="probabilities"></div></section><section class="card"><h2>Experiment input</h2><div id="input" class="kv"></div><h3>Rendered Needle prompt</h3><pre id="rendered"></pre></section></div></section>
<section class="card"><h2>Existing trace</h2><p class="muted">The original trace timeline remains available when you load a trace produced by the CLI.</p></section></main>
<script>
let data=null,pos=0;const $=id=>document.getElementById(id);const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
async function templates(){let ts=await(await fetch('/api/templates')).json();$('templates').innerHTML=ts.map(t=>`<button class="template" data-id="${esc(t.id)}"><b>${esc(t.name)}</b><small>${esc(t.description)}</small></button>`).join('');ts.forEach(t=>document.querySelector(`[data-id="${CSS.escape(t.id)}"]`).onclick=()=>apply(t));if(ts[0])apply(ts[0])}
function apply(t){$('prompt').value=t.prompt;$('system').value=t.system||'';$('tools').value=JSON.stringify(t.tools||[],null,2);$('error').textContent=''}
async function run(){ $('error').textContent='';$('status').textContent='Running…';$('run').disabled=true;try{let tools=JSON.parse($('tools').value||'[]');if(!Array.isArray(tools))throw Error('Tools must be a JSON array.');let r=await fetch('/api/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:$('prompt').value,system:$('system').value,tools,top_k:+$('topk').value,max_new_tokens:+$('maxnew').value})});let j=await r.json();if(!r.ok)throw Error(j.error||'Run failed');data=j;pos=0;$('result').classList.remove('hidden');$('output').textContent=j.generated_text||'(empty)';$('resultSummary').textContent=`${j.events.length} generated steps`;$('rendered').textContent=j.rendered_prompt;$('input').innerHTML=[['prompt',j.query],['system',j.system||'—'],['tools',JSON.stringify(j.tools)],['steps',j.events.length]].map(([k,v])=>`<div class="muted">${esc(k)}</div><div>${esc(v)}</div>`).join('');$('slider').max=Math.max(0,j.events.length-1);draw();render(0);$('status').textContent='Done';$('result').scrollIntoView({behavior:'smooth',block:'start'})}catch(e){$('error').textContent=e.message;$('status').textContent=''}finally{$('run').disabled=false}}
function draw(){let t=$('track');t.innerHTML='';(data?.events||[]).forEach((e,i)=>{let d=document.createElement('button');d.className='dot';d.title=`Step ${i+1}: ${e.chosen.token_text}`;d.style.left=(data.events.length<2?50:i*100/(data.events.length-1))+'%';d.onclick=()=>render(i);t.appendChild(d)})}
function render(i){if(!data?.events?.length)return;pos=Math.max(0,Math.min(i,data.events.length-1));let e=data.events[pos],c=e.chosen;$('slider').value=pos;$('step').textContent=`Step ${pos+1} / ${data.events.length}`;$('chosen').innerHTML=`<b>${esc(c.token_text||'∅')}</b> · ID ${c.token_id} · <code>0x${c.token_id.toString(16).toUpperCase()}</code> · bytes <code>${esc(c.token_bytes_hex||'')}</code> · <b>${(c.probability*100).toFixed(4)}%</b>`;let top=e.top_k||[];$('probabilities').innerHTML='<table class="probability-table"><thead><tr><th>Rank</th><th>Token ID</th><th>Token text</th><th>Token bytes hex</th><th>Probability</th><th>Distribution</th></tr></thead><tbody>'+top.map((x,n)=>`<tr><td>${n+1}</td><td><code>${x.token_id}</code> / <code>0x${x.token_id.toString(16).toUpperCase()}</code></td><td class="token-text">${esc(x.token_text||'')}</td><td class="token-hex"><code>${esc(x.token_bytes_hex||'')}</code></td><td class="probability-value">${(x.probability*100).toFixed(4)}%</td><td><div class="probability-bar"><div class="probability-fill" style="width:${Math.min(100,x.probability*100)}%"></div></div></td></tr>`).join('')+'</tbody></table>';document.querySelectorAll('.dot').forEach((d,n)=>d.classList.toggle('current',n===pos))}
$('run').onclick=run;$('first').onclick=()=>render(0);$('prev').onclick=()=>render(pos-1);$('next').onclick=()=>render(pos+1);$('last').onclick=()=>render(data?.events?.length-1);$('slider').oninput=e=>render(+e.target.value);templates();
</script></body></html>'''
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--host",default="127.0.0.1");p.add_argument("--port",type=int,default=8765);return p
...
def serve(trace_path: Path|None=None,host:str="127.0.0.1",port:int=8765,*,runtime:Any=None)->None:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self)->None:
            ...
            if runtime is None:self._json(503,{"error":"Interactive Run unavailable: start through trace_needle with --checkpoint and --needle-source."});return
            try:
                n=int(self.headers.get("Content-Length","0"));req=json.loads(self.rfile.read(n).decode());result=run_experiment(runtime,req.get("prompt",""),req.get("tools") or [],req.get("system","") or "",max_new_tokens=int(req.get("max_new_tokens",48)),top_k=int(req.get("top_k",5)));self._json(200,result)
    ...
    server=ThreadingHTTPServer((host,port),Handler);print(f"Needle Time Machine UI: http://{host}:{port}/")
...
def main(argv:list[str]|None=None)->int:
    args=build_arg_parser().parse_args(argv);runtime=None
